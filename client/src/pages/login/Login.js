...
const Login = () => {
  ...
  const handleForm = (e) => {
    ...
    if (data !== null) {
      ...
      if (
        state.emp_id.startsWith("op") &&
        /^\d{4}$/.test(state.emp_id.slice(2)) &&
        state.password.length === 6
      ) {
        navigate("../../pages/employee_manage/Operator_view");
        alert("Login Successfully");
      } else if (
        state.emp_id.startsWith("fa") &&
        /^\d{4}$/.test(state.emp_id.slice(2)) &&
        state.password.length === 6
      ) {
  ...
  return (
    <>
      <section class="bg-image card img-fluid ">
        <img
          src={require("../../assets/loginbg1.png")}
          class="card-img-top"
          width="100%"
          style={{ height: "92vh" }}
          alt=""
